...
public class Hospital implements Serializable{
	...
	public StaffMember getStaffMemberByUsernameAndPassword(int username, String password) {
        StaffMember sm = staffMembers.get(username);
        if (sm != null && sm.getPassword().equals(password)) {
            return sm;
        }
        return null;
    }
	public void printAllStaffMembers() {
	    for (Integer id : staffMembers.keySet()) {
	        StaffMember sm = staffMembers.get(id);
	        System.out.println("ID: " + id + ", Password: " + sm.getPassword());
	    }
	}
	 ...
	 public ArrayList<Doctor> getDoctors() {
	    ...
	    public List<String> getDiseaseCodes() {
	        return medicalProblems.values().stream()
	            .filter(mp -> mp instanceof Disease)
	            .map(MedicalProblem::getCode)
	            .collect(Collectors.toList());
	    }

	    public List<String> getFractureCodes() {
	        return medicalProblems.values().stream()
	            .filter(mp -> mp instanceof Fracture)
	            .map(MedicalProblem::getCode)
	            .collect(Collectors.toList());
	    }
	    ...
	    public List<String> getInjuryCodes() {
	        return medicalProblems.values().stream()
	            .filter(mp -> mp instanceof Injury)
	            .map(MedicalProblem::getCode)
	            .collect(Collectors.toList());
	    }
